...
export const generateReview = inngest.createFunction(
    ...
    async ({ event, step }) => {
        ...
        const context = await step.run("retrieve-context", async () => {
            ...
            return await retrieveContext(`${owner}/${repo}`, query)
        })

        const review = await step.run("generate-a-review", async () => {
...
Format your response in markdown.`
        ...
        await step.run("post-comment", async() => {
            if(!token){
                throw new Error("Failed to find token for creating comment")
            }
